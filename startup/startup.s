...
.word _sidata		// data in flash memory
.word _sdata		// start address of .data in RAM
.word _edata		// end address of .data RAM
...
Reset_Handler:
	ldr   sp, =_estack   // load stack pointer in SP register
	bl  ExitRun0Mode	 // to configure power supply
	bl  SystemInit		 // to load system initialization code
	
  // Copy contents from Flash memory into RAM. Copy .data into RAM and .bss is zeroed
  ldr r0, =_sdata
  ldr r1, =_edata
  ldr r2, =_sidata
  movs r3, #0
...
LoopFillZerobss:
  ...
  bcc FillZerobss
    ...
    bl  main		// application entry point
  	bx  lr
.size  Reset_Handler, .-Reset_Handler

// Default Handler when interrupt is received. default is infinite loop running
...
.section .isr_vector, "a",%object

// 15 IRQs: defined by ARM.
// Rest: defined by ST
g_pfnvectors:
.word _estack
.word Reset_Handler
